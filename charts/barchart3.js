function drawChart_a1_v3() {
	...
	d3.csv("../../data/data_barchart_wafflechart_piechart.csv").then(function (data) {
		...
		// Sort data in descending order based on Total_Vaccinations
		data.sort(function (a, b) {
			return b.Total_Vaccinations - a.Total_Vaccinations;
		});

		// Extract the top 20 countries by Total_Vaccinations
		let top20 = data.slice(0, 20);

		// Calculate the sum of Total_Vaccinations for the other countries
		let other = data.slice(20, data.length);
		let sum = d3.sum(other, function (d) { return d.Total_Vaccinations });

		// Use top20 to create the chart
		xScale.domain(top20.map(function (d) { return d.Country; }));
		yScale.domain([0, d3.max(top20, function (d) { return d.Total_Vaccinations; })]);
		...
		g.selectAll(".bar")
			...
			.attr("height", function (d) { return height - yScale(d.Total_Vaccinations); });
	...
	function onMouseOver(d, i) {
		...
		d3.select('#tooltip')
			...
			.html(`
				<h2>${i.Country}</h2>
				<div> Total_Vaccinations: ${i.Total_Vaccinations.toLocaleString()}</div>
			`);
		...
		d3.select(this)
			...
			.attr('y', function (d) { return yScale(d.Total_Vaccinations) - 10; })
			.attr('height', function (d) { return height - yScale(d.Total_Vaccinations) + 10; })
